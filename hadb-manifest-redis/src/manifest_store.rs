...
//!
//! Full manifest stored as msgpack value in a Redis key.
//! CAS via Lua script: atomically read current value, check version, set if match.
//! `meta()` fetches the full value and extracts ManifestMeta (Redis GET is fast enough).
...
///
/// Each manifest uses two Redis keys: a data key and a `{data_key}:version`
/// side-key for cheap CAS comparison in Lua scripts. Both keys use Redis
/// hash tags (`{...}`) so they route to the same slot in Redis Cluster.
///
/// External deletion of one key without the other creates inconsistent state.
/// Only use this store's API to manage manifest keys.
...
impl RedisManifestStore {
    ...
    /// Connect to Redis and create a manifest store with key prefix.
    ...
    /// Returns the data key with a Redis hash tag for Cluster slot routing.
    fn prefixed_key(&self, key: &str) -> String {
        format!("{{{}{}}}", self.prefix, key)
    }

    /// Returns the version side-key, colocated with the data key in Cluster.
...
impl ManifestStore for RedisManifestStore {
    async fn get(&self, key: &str) -> Result<Option<HaManifest>> {
        ...
        match data {
            Some(bytes) => {
                let manifest: HaManifest = rmp_serde::from_slice(&bytes)
    ...
    async fn put(
        ...
        let result: String = match expected_version {
            None => {
                redis::Script::new(CREATE_SCRIPT)
                    .key(&pkey)
                    .key(&vkey)
                    .arg(body.as_slice())
...
mod tests {
    ...
    impl TestFixture {
        ...
        async fn cleanup(self) {
            let mut conn = self.client.clone();
            // Find and delete all keys with our test prefix.
