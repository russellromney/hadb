...
## Phase Signal: ManifestStore Trait

> After: Phase Volt . Before: Phase Forge

Unified manifest for HA coordination and storage state. One fetch tells a node everything: who last wrote, what epoch, and what the current storage state is. Replaces S3 file scanning for follower catch-up discovery.

### Why

Today, followers discover new data by scanning S3 listings for new files. This is O(all files), slow (~50-200ms), and gives no coordination info (who last wrote, what epoch). The manifest collapses coordination + storage state into one small object. Nodes poll ManifestStore on an interval, and one cheap check (~2ms on NATS, ~30ms on S3) tells them if anything changed.

The manifest tracks who last wrote (`leader_id`, `lease_epoch`) for fencing, not for discovery. Leader address for write forwarding is a Dedicated-mode concern handled outside the manifest (via the lease store or service discovery). Shared mode doesn't need leader addresses at all since nodes never communicate directly.
...
pub struct HaManifest {
    pub version: u64,
    pub writer_id: String,      // instance that last published
    pub lease_epoch: u64,       // fencing token
...
`StorageManifest` is extensible. New storage backends (duckblock, graphstream) add variants. The coordination layer (version, writer_id, lease_epoch) is shared across all backends.

### ManifestStore trait

Same pattern as LeaseStore: trait in hadb, implementations in hadb-manifest-s3, hadb-manifest-nats, etc.
...
pub trait ManifestStore: Send + Sync {
    ...
    /// Publish a new manifest. CAS on expected_version.
    /// None = must not exist (first publish).
    async fn put(
        &self,
        key: &str,
        manifest: &HaManifest,
    ...
    /// Cheap metadata check. Returns version + leadership info
    /// without fetching the full storage payload.
    async fn meta(&self, key: &str) -> Result<Option<ManifestMeta>>;
}
```

Three methods:
- `get`: full manifest fetch. Used on catch-up (need storage details to know what to pull).
- `put`: CAS publish. Fencing on top of the lease. Prevents stale writers from publishing even if lease logic has a bug.
- `meta`: lightweight HEAD. Nodes poll this on interval. Returns version + writer info for fencing. For NATS/Redis this is ~1-2ms. For S3, a HeadObject with version in custom metadata headers avoids the full GET.

### ManifestData serialization

`HaManifest` serializes to msgpack (compact, fast) with JSON fallback for debugging. The `ManifestStore` implementations treat the body as opaque bytes with the version pulled out for CAS and meta checks.

For S3ManifestStore: store `version` in S3 object metadata header (`x-amz-meta-manifest-version`), so `meta()` is a HeadObject (no body download). Full manifest in the object body.

For NatsManifestStore: store full manifest as NATS KV value, revision = version for CAS. `meta()` reads just the key metadata (revision + a small header prefix).

### Signal-a: Types and trait in hadb (DONE)

- `HaManifest`, `ManifestMeta`, `StorageManifest` structs in `hadb/src/manifest.rs`
- `ManifestStore` trait in `hadb/src/manifest.rs`
- `InMemoryManifestStore` for testing (same pattern as `StatefulLeaseStore` in tests)
- Serde derive for msgpack + JSON serialization
- 30 tests: CAS semantics, meta, put/get round-trip, version mismatch, boundary values, contract enforcement, full field verification

### Signal-b: S3ManifestStore (hadb-manifest-s3)

- `S3ManifestStore` implements `ManifestStore`
- `get`: GetObject, deserialize msgpack body
- `put`: PutObject with `x-amz-meta-manifest-version` header, If-Match on ETag for CAS. Writer computes next version client-side.
- `meta`: HeadObject, read version + writer_id + lease_epoch from metadata headers. Trust the header, no body verification.
- Same bucket as page groups/changesets, key = `{prefix}{db_name}/manifest`
- Tests against real S3 (gated by env var)

### Signal-c: NatsManifestStore (hadb-manifest-nats)

- `NatsManifestStore` implements `ManifestStore`
- NATS JetStream KV, same pattern as `NatsLeaseStore`
- `get`: KV get, deserialize value
- `put`: KV create (if expected_version None) or update with revision CAS
- `meta`: KV get, deserialize, return ManifestMeta (NATS KV has no partial read)
- Tests against real NATS (gated by NATS_URL env var)

### Signal-d: EtcdManifestStore (hadb-manifest-etcd)

- `EtcdManifestStore` implements `ManifestStore`
- Same transaction pattern as `EtcdLeaseStore`: Txn with ModRevision compare for CAS
- `get`: etcd get, deserialize msgpack value
- `put`: Txn with CreateRevision==0 (if expected_version None) or ModRevision compare
- `meta`: etcd get, deserialize, return ManifestMeta
- Prefixed keys to avoid collisions
- Tests against real etcd (gated by ETCD_ENDPOINTS env var)

### Signal-e: RedisManifestStore (hadb-manifest-redis)

- `RedisManifestStore` implements `ManifestStore`
- Lua script for atomic CAS: GET current version, compare, SET if match
- `get`: GET key, deserialize msgpack value
- `put`: Lua script: EVAL (get current, check version, set if match, return result)
- `meta`: GET key, deserialize full manifest, return ManifestMeta (Redis GET is fast enough)
- Tests against real Redis (gated by REDIS_URL env var)
...
```
hadb                    -- traits: LeaseStore, ManifestStore, Replicator, Executor, StorageBackend
hadb-manifest-s3        -- S3ManifestStore
hadb-manifest-nats      -- NatsManifestStore
hadb-manifest-etcd      -- EtcdManifestStore
hadb-manifest-redis     -- RedisManifestStore
```
