...
//!
//! Full manifest stored as msgpack value in NATS KV. NATS KV revisions
//! provide natural CAS semantics. `meta()` fetches the full value and
//! extracts ManifestMeta (NATS KV has no partial-read API, but reads
//! are ~1-2ms so this is fine).
...
impl NatsManifestStore {
    ...
    fn decode_entry(entry: Option<kv::Entry>) -> Result<Option<Manifest>> {
        match entry {
            Some(e) => match e.operation {
                kv::Operation::Put => {
                    let manifest: Manifest = rmp_serde::from_slice(&e.value)
...
impl ManifestStore for NatsManifestStore {
    ...
    async fn put(
        ...
        match expected_version {
            None => match self.store.create(key, Bytes::from(body)).await {
...
mod tests {
    ...
    impl TestFixture {
        /// Requires `NATS_URL` env var pointed at a live NATS JetStream
        /// server. Panics loudly if unset — silent skips let broken
        /// tests look green.
        async fn new() -> Self {
            let url = std::env::var("NATS_URL").expect(
                "NATS_URL must be set for turbodb-manifest-nats integration tests \
                 (e.g. nats://127.0.0.1:4222 against the dev stack)",
            );
